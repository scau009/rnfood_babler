...
class WechatMpPayNotifyService
{
    ...
    public function handleNotify()
    {
        return $this->weChatMpPayService->getApp()->handlePaidNotify(function ($message,$fail){
            ...
            $ttrade = $this->weChatMpPayService->getApp()->order->queryByOutTradeNumber($message['out_trade_no']);

            $this->logger->info("微信支付流水订单：".json_encode($ttrade));
            ...
            if ($message['return_code'] === 'SUCCESS') { // return_code 表示通信状态，不代表支付状态
                // 用户是否支付成功
                if ($message['result_code'] && $message['result_code'] === 'SUCCESS') {
